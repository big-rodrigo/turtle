<script lang="ts">
	...
	onMount(() => {
		...
		function init() {
			...
			pivot.position.set(3.0, -0.5, 0);
			pivot.rotation.y = Math.PI / 4;
			scene.add(pivot);

			const ANIMS = [
				{ in: 'yasuo_dance_in.anm', loop: 'yasuo_dance_loop.anm' },
				{ loop: 'yasuo_idle1.anm' },
				{ loop: 'yasuo_joke.anm' },
				{ loop: 'yasuo_laugh.anm' },
				{ loop: 'yasuo_taunt.anm' },
				{ loop: 'yasuo_recall_loop.anm' },
			];
			let animIdx = 0;
			...
			loader.load('/models/yasuo/scene.gltf', (gltf) => {
				if (destroyed) return;
				...
				pivot.scale.setScalar((2 / maxDim) * 0.6);
				...
				function warmupTick() {
					...
					if (--warmup > 0) {
						...
						requestAnimationFrame(() => { if (!destroyed) opacity = 0.8; });
					}
				}
				requestAnimationFrame(warmupTick);
			});
